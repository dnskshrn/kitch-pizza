...
|----------|-------------------|
| Конфиг брендов | `src/brands/index.ts` — `BrandConfig`, `brands[]`, `getBrandByHost`, `getBrandBySlug`; configured slugs: `kitch-pizza`, `losos`, `the-spot`. `getBrandByHost()` нормализует `Host` без порта (`thespot.md:3000` → `thespot.md`, `192.168.50.137:3000` → `192.168.50.137`). Временно для мобильного локального теста The Spot `devDomain` = `192.168.50.137`. |
...
│   │   ├── page.tsx              # Home: PromotionsSlider + MenuSection; The Spot: pt-3 mobile / md:pt-2, категории после промо + floating cart
│   │   └── checkout/
...
│   └── layout.tsx
├── components/client/
│   ├── client-chrome.tsx         # оболочка витрины: на /checkout* без TopNav/MainHeader/MenuCategoryBar; StorefrontHaptics
...
| **The Spot storefront** | Активируется при `data-brand="the-spot"` / slug `the-spot`. **`ClientChrome`** скрывает `TopNav` и глобальный Kitch `MenuCategoryBar`; на главной `page.tsx` категории рендерятся после промо. **Mobile header:** белая pill `63px` с `the-spot-logo.svg`, адресом (`str. Dacia 35` fallback) и burger; sticky header держится ниже modal overlay (`z-30`), чтобы затемняться при корзине/попапах. **Tablet/Desktop header (`md+`):** отдельная белая rounded-full/pill-плашка на фоне `#f5f2f0`: логотип, адресная pill-кнопка, nav (`Меню` / `Акции` / `Контакты`), RU/RO toggle на `lg+`, телефон на `xl+`, burger на `md/lg`. **Промо (`PromotionsSlider`):** горизонтальный слайдер с **scroll-snap** и шаговым свайпом; **стрелки** внутри галереи на всех размерах (если баннеров > 1); **автослайд 5 с**, таймер **сбрасывается** при взаимодействии (клик стрелок, свайп, скролл); навигация **циклическая** (после последнего — первый); индикаторы-точки **убраны**; на mobile высота баннера стабилизирована для iOS Safari (`clamp` + ширина карточки `100%`); на `md+` карточки `16/7`, стрелки сдвинуты внутрь (`left-4` / `right-4`); секция промо с **`md:mb-5`**; если баннеров нет — placeholders как раньше (`md+` три карточки `16/9`). **Категории:** brand-specific `TheSpotCategoryBar`: mobile горизонтальные pill-чипы, `md+` sticky белая pill-полоса с категориями и оранжевой корзиной; active `#f25130`/white; клик/scroll-spy синхронизированы с `MenuSection`; sticky z-index также ниже overlay (`z-30`). **Меню:** mobile 2 колонки; `md` — 3 колонки, `xl` — 4 колонки; белые image cards `aspect-square rounded-[12px]`, текст `#242424`, muted `rgb(36 36 36 / 60%)`, price pill `#ffebe7` / `#b7614f`, hover на price pill в `#f25130`/white, формат `MDL`. **Floating cart:** только mobile (`md:hidden`) fixed bottom glass pill, CTA `#f25130`, сумма через `selectCartSubtotal`. |
...
| **PromotionsSlider** | `promotions-slider.tsx`: язык из `localStorage` `lang`; RU/RO картинки; **`use-window-width`** (SSR-ширина 1200 до гидратации). **Kitch и The Spot:** при нескольких баннерах — стрелки на всех breakpoints, **циклическая** прокрутка, **автослайд 5 с** для обоих брендов, сброс таймера при взаимодействии; **scroll-snap**, шаговый свайп. Геометрия карточек: Kitch — формулы `cardWidthExpr`; The Spot — `theSpotCardWidthExpr` (mobile `100%` + фикс высоты для iOS), `md+` несколько колонок и gap `20px`; The Spot — доп. **`md:mb-5`** у секции. |
...
- **`PromotionsSlider`:** стрелки при нескольких баннерах на всех breakpoints (Kitch и The Spot); **автослайд 5 с** с **сбросом таймера** при взаимодействии; **циклическая** навигация; **scroll-snap** и шаговый свайп; точки убраны; правки высоты/ширины карточек для **iOS Safari**; у The Spot секция промо с **`md:mb-5`**.
- **Checkout The Spot:** панель формы **`storefront-checkout-form-panel`** — прозрачный фон на mobile, **белые** поля **`storefront-modal-field`**, без «карточки» вокруг всей формы на тёплом фоне; выровнены вертикальные отступы и блок оплаты; горизонтальные отступы как у **`client-container`**; зарезервирована высота под fixed pill шапки на mobile.
...
- Локальная проверка The Spot: добавить в `/etc/hosts` `127.0.0.1 thespot.md www.thespot.md`, затем открыть **`http://thespot.md:3000/`**. Для теста с телефона в одной Wi-Fi сети сейчас временно прописан `devDomain: "192.168.50.137"` у The Spot; запускать dev можно как **`npm run dev -- -H 192.168.50.137`** и открывать **`http://192.168.50.137:3000/`**. Важно: браузер отправляет `Host` с портом, поэтому `getBrandByHost()` нормализует host без порта.
