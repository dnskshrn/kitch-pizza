...
- **POS:** `/pos/*`, отдельный вход по PIN, смены, создание и редактирование заказов; **экран кухни KDS** — `/pos/kds` (все заказы со статусом **`cooking`** по всем брендам; бейдж бренда на карточке из join **`brands.slug`**).
...
│   ├── (admin)/admin/   # админка; `layout.tsx` + `AdminShell` / `AdminSidebar` (shadcn offcanvas); `loading.tsx` на корне; `staff/` (персонал, service role); `inventory/`, `orders/`, `menu/`; `legacy-menu-sizes.ts`; condiments/
...
├── lib/
│   ├── actions/         # server actions; `staff/staff-actions.ts` — админка персонала (service role)
...
- **Склад** (`/admin/inventory/*`) и **персонал** (`/admin/staff`): данные **общие для всех брендов** — в server actions и `page.tsx` этих разделов **нет** фильтрации выборок и записи по `getAdminBrandId()` для таблиц **`ingredients`**, **`semi_finished`**, **`stock_audits`**, **`suppliers`**, **`supply_orders`** (и связанных строк). Типы и колонки `brand_id` в БД при необходимости остаются; вставки/обновления в этих разделах не передают `brand_id` из cookie.
...
| `/admin/staff` | персонал: CRUD сотрудников (`staff`), PIN через **`bcryptjs`** в server actions; чтение/запись с **`createServiceRoleClient()`** (не светить **`pin_hash`** в anon); для роли **курьер** — генерация deep-link Telegram (`generateTelegramLink`, env **`TELEGRAM_BOT_USERNAME`**) |
...
| `/admin/inventory/tech-cards` | техкарты: **`product_recipes`** + мета **`product_recipe_meta`** (выход блюда **`output_qty`**, **`output_unit`**; тип **`ProductRecipeMeta`** в `database.ts`). Таблица на странице: по каждой **активной позиции меню выбранного в переключателе бренда** — одна строка без размеров или по строке на вариант при **`has_sizes`** (`getAdminBrandId` на **`menu_items`**); выпадающие ингредиенты и п/ф в диалоге — **все записи склада** (без фильтра по бренду). **`InventorySearch`** по названию блюда и варианту (`tech-cards-table.tsx`). Диалог **`tech-card-dialog.tsx`** (`sm:max-w-4xl`): ингредиенты и п/ф с `avg_cost` / себестоимостью за единицу выхода п/ф, сводка вход/выход/потери/себестоимость блюда, количества в UI в **кг/л/шт**. **`saveProductRecipe`** — проверка, что **`menu_item_id`** принадлежит бренду из **`getAdminBrandId()`**, затем delete+insert рецепта и upsert/удаление меты при **`output_qty > 0`** / сбросе. Для **`variant_id = null`** в запросах Supabase — **`.is('variant_id', null)`**. |
...
- **Склад (inventory):** server actions рядом со страницами: `src/app/(admin)/admin/inventory/**/actions.ts` (техкарты **`saveProductRecipe`** с **`product_recipe_meta`** и проверкой **`menu_items`** по бренду, поставщики, ингредиенты, **поставки `createSupplyOrder`**, **инвентаризации** `createAudit` / `confirmAudit` / `updateAuditItemActualQty`, и т.д.). Операции по таблицам **`ingredients`**, **`semi_finished`**, **`stock_audits`**, **`suppliers`**, **`supply_orders`** **не** привязаны к **`getAdminBrandId()`**. После поставки — `revalidatePath` в том числе для остатков и ингредиентов. На клиенте списки разделов **`/admin/inventory/*`** с таблицами используют единое поле поиска **`InventorySearch`** (`src/components/admin/inventory-search.tsx`); фильтрация строк без запросов к серверу (имена и релевантные текстовые поля по разделам); на **Остатках** поиск накладывается на уже отфильтрованный по наличию набор строк.

## POS

- URL: `/pos/login` для PIN, `/pos` для рабочей зоны; **`/pos/kds`** — Kitchen Display (**все заказы `status = cooking`**, без фильтра по **`brand_id`**).
...
- **Шаг «Детали»:** имя, телефон, адрес и структурные поля адреса при вводе сразу обновляют объект заказа в списке слева через **`updateOrderLocalState`**; **`updateOrderDetailsPos`** вызывается с **debounce 600 ms** после паузы во вводе. При обновлении **`listOrder`** с панели и **`!form.formState.isDirty`** выполняется **`form.reset`** из данных заказа списка (без отдельного запроса только ради полей формы). Переключатели **тип заказа (доставка / самовывоз)** и **способ оплаты** сохраняются **сразу** по клику. Кнопка **«Отправить бегунок»** на шаге 3: валидация Zod; **`persistCartToServer`**; **`runDetailsSaveToServer`**; **`sendPosDraftToKitchen`** — **`UPDATE`** черновика (**`status = cooking`**, **`cooking_started_at`**, **`updated_at`**). После успеха: **`refetchOrders`**, **`toast.success`**, сброс блокировок; **мастер остаётся открытым** на том же **`orderId`**. Защита от двойного нажатия: ref + **`submitting`** / **`runnerBusy`**; при **`listOrder.status === cooking`** — состояние «Бегунок отправлен». Уведомления об ошибках — **Sonner**.
- **«Отправить бегунок» на шаге 2 (корзина):** кнопка в панели корзины рядом с **«К деталям»**: пустая корзина → ошибка; **`persistBrandOrError`**, **`persistCartToServer`**, **`sendPosDraftToKitchen`**; после успеха — как на шаге 3 (**`refetchOrders`**, тост), без нового черновика и без сброса шага.
...
- `orders`, `order_items` — **`orders.status`**: канон перечислен в **`OrderStatus`** (`src/types/database.ts`: `draft` · `new` · `confirmed` · `cooking` · `ready` · `delivery` · `done` · `cancelled` · `rejected`). У **`orders`** также **`cooking_started_at`** (timestamptz, см. миграцию **`20260506120000_orders_cooking_started_at.sql`**) для таймера KDS. Опционально в типах и схеме: поля доставки/курьера (**`courier_id`**, **`courier_assigned_at`**, **`delivered_at`**, **`delivery_lat`**, **`delivery_lng`** и т.п. по мере использования в приложении). В приложении поле **`user_birthday`** у заказов не используется (колонка в типах/селектах убрана). У **`order_items`**: **`variant_id`** (nullable, FK на **`menu_item_variants`**), **`size`** — текстовый снимок подписи варианта на момент заказа; для старых строк допустимы **`s`** / **`l`** (только отображение, без перезаписи логики).
...
- **Склад (админка):** не в `src/lib/actions/`, а в `src/app/(admin)/admin/inventory/**/actions.ts` — см. раздел **Админка** / таблицу маршрутов **Склад**. **Персонал (админка):** `src/lib/actions/staff/staff-actions.ts`.
...
`POS_SESSION_SECRET` должен быть не короче 32 символов. `PBX_WEBHOOK_TOKEN` — общий секрет с телефонией (поле `crm_token` в теле webhook). **`TELEGRAM_BOT_USERNAME`** — имя бота без `@` для ссылок привязки Telegram в **`/admin/staff`** (курьеры); `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` — уведомления о новых заказах с витрины (**`create-order`**).
