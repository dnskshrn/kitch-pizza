...
import { getMenuItemToppingGroups } from "../menu/actions"
import {
  createCondimentMenuItem,
  setCondimentMenuItemToppingGroups,
  updateCondimentMenuItem,
  type CondimentMenuItemPayload,
} from "./actions"
...
type Props = {
  ...
  mode: "create" | "edit"
  item: CondimentMenuItemRow | null
  condimentCategoryId: string
  toppingGroups: ToppingGroup[]
}

export function CondimentItemDialog({
  open,
  onOpenChange,
  mode,
  item,
  ...
  const [isDefaultCondiment, setIsDefaultCondiment] = useState(true)
  const [condimentDefaultQty, setCondimentDefaultQty] = useState(0)
  ...
  useEffect(() => {
    ...
    if (mode === "edit" && item) {
      ...
      setIsDefaultCondiment(item.is_default_condiment ?? false)
      setCondimentDefaultQty(
        Math.min(
          10,
          Math.max(
            0,
            typeof item.condiment_default_qty === "number" &&
              Number.isFinite(item.condiment_default_qty)
              ? Math.trunc(item.condiment_default_qty)
              : 0,
          ),
        ),
      )
      ...
      setIsActive(true)
      setSortOrder(0)
      setIsDefaultCondiment(true)
      setCondimentDefaultQty(0)
      setSelectedGroupIds([])
      setAddDiscount(false)
      setDiscountPercentStr("")
      setTagValue(TAG_NONE)
    }
  }, [open, mode, item, condimentCategoryId])
...
function parseGrams(s: string): number | null | "invalid" {
  ...
  function handleSave() {
    ...
    const cdqRaw = Number(condimentDefaultQty)
    let condiment_default_qty = Number.isFinite(cdqRaw)
      ? Math.trunc(cdqRaw)
      : 0
    condiment_default_qty = Math.min(10, Math.max(0, condiment_default_qty))

    let payload: CondimentMenuItemPayload
    ...
    if (hasSizes) {
      ...
      payload = {
        ...
        discount_percent,
        tag,
        is_default_condiment: isDefaultCondiment,
        condiment_default_qty,
      }
    } else {
      const p = parseLei(priceLei)
      if (p === null) {
    ...
    startTransition(async () => {
      try {
        if (mode === "create") {
          const newId = await createCondimentMenuItem(payload)
          await setCondimentMenuItemToppingGroups(newId, selectedGroupIds)
        } else if (item) {
          const includedRaw =
            (item as MenuItem & { included_items?: unknown }).included_items ??
            []
          await updateCondimentMenuItem(item.id, payload, includedRaw)
          await setCondimentMenuItemToppingGroups(item.id, selectedGroupIds)
        }
        onOpenChange(false)
      } catch (e) {
        console.error(e)
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        ...
        <div className="grid gap-4 py-2">
          ...
          <div className="flex items-center gap-2">
            <Switch
              id="cond-default-order"
              checked={isDefaultCondiment}
              onCheckedChange={setIsDefaultCondiment}
            />
            <Label htmlFor="cond-default-order">
              Добавить в заказ автоматически
            </Label>
          </div>
          <div className="grid gap-2">
            <Label htmlFor="cond-default-qty">Количество по умолчанию</Label>
            <Input
              id="cond-default-qty"
              type="number"
              min={0}
              max={10}
              step={1}
              value={condimentDefaultQty}
              onChange={(e) => {
                const raw = Number(e.target.value)
                const n = Number.isFinite(raw) ? Math.trunc(raw) : 0
                setCondimentDefaultQty(Math.min(10, Math.max(0, n)))
              }}
            />
          </div>
          <div className="flex items-center gap-2">
