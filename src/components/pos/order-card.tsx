...
export function OrderCard({
  ...
  const scheduledLabel = isKdsScheduledOrder(order.scheduled_time)
    ? formatScheduledTimeLabel(order.scheduled_time!)
    : null
  const totalLabel = formatMdl(order.total ?? 0)
  const showCourier =
    order.delivery_mode === "delivery" &&
    order.status === "delivery" &&
    Boolean(order.courier_id)
  const courierName = order.courier_name?.trim() || "Курьер"

  return (
    <div
      className={cn(
        "flex flex-col gap-0 rounded-lg border-2 bg-white px-4 py-2.5 shadow-none transition-colors",
        isSelected
    ...
    >
      ...
      >
        {/* ── Строка 1: иконка · бренд · время · номер · статус ── */}
        <div className="flex items-center gap-1.5">
          ...
          <PosBrandMark
            ...
            className="[&_img]:max-h-3 [&_img]:max-w-[72px]"
          />

          <span className="font-mono text-[13px] font-bold tabular-nums text-[#242424]">
            {orderTime}
          </span>
          ...
          {scheduledLabel ? (
            <span
              className="flex items-center gap-1 rounded-md border px-1.5 py-0.5 text-[11px] font-semibold tabular-nums"
              style={{
        ...
        {promoTrim !== "" || order.discount > 0 ? (
          <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 pl-[18px] text-[11px] leading-snug text-emerald-700">
            {promoTrim !== "" ? (
        ...
        {order.delivery_mode === "aggregator" ? (
          <div className="flex items-center gap-1.5">
            <span className="text-[13px] font-bold uppercase tracking-wide text-orange-600">
              GLOVO
            </span>
            <span className="flex-1" />
            <span className="font-mono text-[13px] font-bold tabular-nums text-[#242424]">
          ...
          <div className="flex min-w-0 items-center gap-1.5 rounded-lg bg-[#f2f2f2] px-2.5 py-1">
        ...
        {showCourier ? (
          <div className="flex min-w-0 items-center gap-1.5">
            <CircleUser className="size-3.5 shrink-0 text-[#808080]" aria-hidden />
            <span className="min-w-0 truncate text-[13px] text-[#242424]">
              {courierName}
            </span>
          </div>
        ) : null}
        ...
        {order.delivery_mode !== "aggregator" ? (
          <div className="flex min-w-0 items-center gap-1.5">
            ...
            <span className="flex-1" />
            <span className="shrink-0 font-mono text-[13px] font-bold tabular-nums text-[#242424]">
              {totalLabel}
            </span>
          </div>
        ) : null}
