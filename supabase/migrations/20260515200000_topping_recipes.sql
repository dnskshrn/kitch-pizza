-- Состав топпинга для списания со склада при использовании в заказах
CREATE TABLE public.topping_recipes (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  topping_id uuid NOT NULL REFERENCES public.toppings (id) ON DELETE CASCADE,
  ingredient_id uuid REFERENCES public.ingredients (id) ON DELETE RESTRICT,
  semi_finished_id uuid REFERENCES public.semi_finished (id) ON DELETE RESTRICT,
  quantity numeric NOT NULL,
  quantity_gross numeric,
  CONSTRAINT topping_recipes_one_component CHECK (
    (ingredient_id IS NOT NULL AND semi_finished_id IS NULL)
    OR (ingredient_id IS NULL AND semi_finished_id IS NOT NULL)
  ),
  CONSTRAINT topping_recipes_quantity_positive CHECK (quantity > 0)
);

CREATE INDEX idx_topping_recipes_topping_id ON public.topping_recipes (topping_id);

ALTER TABLE public.topping_recipes ENABLE ROW LEVEL SECURITY;

CREATE POLICY "topping_recipes_authenticated_all"
  ON public.topping_recipes
  FOR ALL
  TO authenticated
  USING (true)
  WITH CHECK (true);
